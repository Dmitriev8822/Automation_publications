...
Дополнительно `app/main.py` регистрирует в Telegram-боте кнопку ручной публикации и кнопку `Контент план`. Ручная публикация теперь готовит черновик через `app/service.py`, показывает его на согласование и только после кнопки `Принять` публикует пост в группу и сохраняет запись в БД; пользователь может отменить черновик, перегенерировать текст и изображение или отдельно перегенерировать картинку. Меню `Контент план` позволяет посмотреть сохраненные запланированные пункты, удалить выбранный пункт или весь план, отредактировать выбранный пункт или весь план через AI-инструкцию пользователя, составить новый КП или настроить напоминания. Диалог составления контент-плана принимает свободное описание, вызывает `AIClient.generate_content_plan()`, показывает структурированный результат и продолжает чат с учетом истории уточнений до нажатия кнопки согласования. После согласования план сохраняется через `ContentPlanRepository`. Планировщик регистрирует отдельные `date`-jobs для пунктов согласованных планов и в согласованное время вызывает `publish_due_content_plan_items()`. Кнопка `Напоминания` находится внутри меню контент-плана: бот сначала показывает текущий таймер, затем предлагает переустановить его готовыми вариантами за 5, 15, 30, 60 минут или через пользовательское число минут, либо отключить настройку. Включенная настройка хранится в БД и применяется ко всем еще запланированным пунктам: уже существующим и новым. Автопубликация по времени также показывает предпубликационное напоминание при включенных напоминаниях: пользователь может подтвердить, что публикация остается в расписании, отменить пункт, перегенерировать текст и изображение или отдельно перегенерировать описание картинки. Если пользователь ничего не отвечает, пункт также остается в расписании и публикуется по таймеру; публикация отменяется только явным отказом. Пользовательские времена без явного timezone трактуются через `APP_TIMEZONE` и сохраняются для планирования в этом timezone. Периодическая новостная публикация раз в `PUBLISH_INTERVAL_MINUTES` оставлена как legacy-заготовка, но не используется в актуальном runtime.
...
| database | `app/database.py` | SQLite, SQLAlchemy, репозитории публикаций, контент-планов и постоянной настройки напоминаний |
...
Черновик используется ручным сценарием: он хранится в диалоге Telegram до решения пользователя и не публикуется без явного согласования.
