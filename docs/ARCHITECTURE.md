...
быстрые тесты без внешних API
  ↓
app/scheduler.py
  ↓ каждые CONTENT_PLAN_POLL_INTERVAL_MINUTES минут
app/service.py
  ↓
...
Дополнительно `app/main.py` регистрирует в Telegram-боте кнопку ручной публикации и кнопку `Контент план`. Ручная публикация вызывает тот же бизнес-сценарий `app/service.py`, что и scheduler. Диалог контент-плана принимает свободное описание, вызывает `AIClient.generate_content_plan()`, показывает структурированный результат с кнопками перегенерации и согласования, а после согласования сохраняет план через `ContentPlanRepository`. Планировщик при каждом запуске в первую очередь публикует наступившие пункты согласованных планов через `publish_due_content_plan_items()`. Регулярная публикация свежих новостей в scheduler оставлена в коде, но по умолчанию выключена настройкой `ENABLE_SCHEDULED_NEWS=false`; при включении она выполняется после контент-плана и не чаще `PUBLISH_INTERVAL_MINUTES`. Ручная публикация новости через Telegram-кнопку остается доступной.
