...
Так тестируются успешный парсинг новостей и постов, парсинг ссылки на изображение, декодирование base64/data URL изображения, игнорирование несуществующего `file_path`, пустой ответ, невалидный JSON и отключение изображения без сетевых запросов.
...
`AIClient.generate_content_plan(description: str) -> ContentPlan` принимает свободное описание пользователя и возвращает структурированный `ContentPlan`. Метод использует тот же OpenRouter chat completions endpoint и JSON Schema response format. Ожидаемый ответ имеет вид:
