...
System prompt задаёт роль генератора безопасного editorial image asset и явно запрещает возвращать только переписанный текстовый prompt вместо изображения. User prompt передаёт заголовок, текст поста и `image_prompt`. Ожидаемый JSON должен содержать хотя бы одно из полей `base64_data`, `data`, `url` или `file_path`, а также опциональный `mime_type`.

Если OpenRouter/модель возвращает `base64_data` или data URL в поле `data`, клиент декодирует строку из base64 в реальные байты изображения перед передачей `ImageAsset` в Telegram. Это важно: отправка base64-строки как обычного UTF-8 текста приводит к невалидному файлу изображения.
...
Так тестируются успешный парсинг новостей и постов, парсинг ссылки на изображение, декодирование base64/data URL изображения, пустой ответ, невалидный JSON и отключение изображения без сетевых запросов.
