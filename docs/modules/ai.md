...
тот же запрос с более совместимым `response_format = {"type": "json_object"}`. Для экономии повторных неуспешных
попыток клиент запоминает HTTP 400 Bad Request по паре `model + schema_name`: после трёх таких отказов он перестаёт
пробовать `json_schema` для этой пары в рамках текущего процесса и сразу использует `json_object`. Это сохраняет работу
сервиса с моделями, у которых structured outputs временно недоступны или отличаются по требованиям, но не тратит
каждый последующий запуск на заведомо отклоняемую strict-схему.
...
- Ошибки HTTP-клиента и статусов OpenRouter нормализуются в `OpenRouterRequestError` с HTTP-статусом и, если доступно, текстом ошибки из ответа OpenRouter. Перед окончательной ошибкой клиент делает fallback с `json_object`, если первый запрос с `json_schema` был отклонён. Повторные HTTP 400 Bad Request для одной пары `model + schema_name` запоминаются; после трёх отказов клиент в текущем процессе сразу начинает с `json_object` для этой пары.
...
- `OPENROUTER_MODEL` — модель для chat completions; дефолт `openai/gpt-4.1-mini`. Если в `.env` указан суффикс `:online` (например `openai/gpt-4.1-mini:online`), клиент передаёт его в OpenRouter без изменений, чтобы не отключить online/search-grounded режим модели;
...
Так тестируются успешный парсинг новостей и постов, подключение `openrouter:web_search` только к поиску новостей, сохранение суффикса модели `:online`, fallback с `json_schema` на `json_object`, отключение повторных попыток `json_schema` после нескольких HTTP 400 Bad Request, парсинг ссылки на изображение, декодирование base64/data URL изображения, игнорирование несуществующего `file_path`, пустой ответ, невалидный JSON и отключение изображения без сетевых запросов.
...
- `regenerate_content_plan(plan, instruction="") -> ContentPlan` — перестраивает весь согласованный контент-план по свободной инструкции пользователя;
