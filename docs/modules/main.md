...
`build_runtime()` дополнительно создает `ContentPlanRepository`, регистрирует `TelegramPublisher.register_content_plan_handler(...)` с обертками сохранения, удаления/AI-редактирования отдельного пункта и удаления/AI-редактирования всего плана и после каждого сохраненного плана обновляет `date`-jobs через `add_content_plan_item_jobs(...)`. При старте приложения уже сохраненные пункты со статусом `scheduled` берутся из `get_scheduled_item_slots()` и сразу попадают в scheduler. Публикация происходит в согласованное время каждого пункта, а не общей проверкой раз в полчаса. Плановый job передает в `publish_due_content_plan_items()` только Telegram publisher и репозиторий, без `AIClient`, поэтому согласованный текст КП не перегенерируется непосредственно перед отправкой в группу.
...
Reminder-job перед отправкой уведомления заново читает пункт контент-плана из БД и проверяет, что его статус всё еще `scheduled`. Если пользователь уже отменил пункт или публикация завершилась до срабатывания reminder-job, уведомление не отправляется повторно; одобрение не меняет статус пункта и не снимает его с расписания.
