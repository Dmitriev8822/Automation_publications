...
- `add_content_plan_reminder_jobs(scheduler, job_func, scheduled_items, minutes_before) -> None` — добавляет или заменяет `date`-jobs вида `content_plan_reminder_<id>` на время `scheduled_at - minutes_before`.
- `remove_content_plan_reminder_jobs(scheduler) -> None` — удаляет все jobs напоминаний, не трогая jobs публикаций; используется при отключении или переустановке постоянной настройки.
...
5. Читает постоянную настройку напоминаний из `ReminderSettingsRepository`: если она включена, регистрирует reminder-jobs для всех текущих scheduled-пунктов; если выключена, удаляет reminder-jobs.
