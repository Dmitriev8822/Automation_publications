...
- интервал берется из `interval_minutes`; для runtime сейчас передается `CONTENT_PLAN_POLL_INTERVAL_MINUTES`, чтобы контент-план проверялся часто;
...
Scheduler пишет INFO-логи при создании расписания, регистрации job, старте и успешном завершении каждого планового запуска. Первый запуск выполняется сразу после старта scheduler, затем повторяется с интервалом, переданным в `create_scheduler()`. В runtime это `content_plan_poll_interval_minutes` с дефолтом 1 минута, чтобы пункты контент-плана выходили вовремя. Исключения job логируются через `logger.exception`, но не останавливают последующие запуски.
